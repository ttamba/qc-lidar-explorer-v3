...
export default function MapView(props: Props) {
  ...
  function getTileFromRenderedFeature(feature: unknown): RuntimeTileFeature | null {
    ...
    const dataset = props.normalized_product === "lidar" || props.__dataset === "lidar" ? "lidar" : props.normalized_product === "mnt" || props.__dataset === "mnt" ? "mnt" : null;
  ...
  function fitMapToAoi(map: Map, aoi: AoiFeature | null) {
    ...
    map.fitBounds(bounds, { padding: { top: 60, right: 60, bottom: 60, left: 60 }, duration: 700, maxZoom: 15 });
  }
  ...
  function getOrCreateWorker() {
    ...
    try {
      workerRef.current = new Worker(new URL("../selection/intersect.worker.ts", import.meta.url), { type: "module" });
  ...
  async function runIntersectWithWorker(aoi: AoiFeature, tiles: RuntimeTileFeature[]): Promise<Set<string>> {
  ...
  async function refreshSelection(map: Map, lidarTiles: RuntimeTileFeature[], mntTiles: RuntimeTileFeature[]) {
    ...
    try {
      ...
      const selectedLidarTiles = lidarTiles.filter((tile) => selectedLidarIds.has(tile.id));
      const selectedMntTiles = mntTiles.filter((tile) => selectedMntIds.has(tile.id));
      ...
      onSelectionChangeRef.current([...selectedLidarTiles, ...selectedMntTiles]);
    } catch (error) {
      ...
      try {
        ...
        applySelectionState(map, "lidar", selectedLidarIds, false);
        applySelectionState(map, "mnt", selectedMntIds, false);
        setSelectedSourceData(map, SRC_LIDAR_SELECTED, selectedLidarFallback as RuntimeTileFeature[]);
        setSelectedSourceData(map, SRC_MNT_SELECTED, selectedMntFallback as RuntimeTileFeature[]);
        onSelectionChangeRef.current([...selectedLidarFallback, ...selectedMntFallback]);
      } catch (fallbackError) {
  ...
  async function refreshTiles(map: Map, options?: { reloadData?: boolean }) {
    ...
    const activeYear = activeDataset === "lidar" ? yearFilterRef.current.lidar : yearFilterRef.current.mnt;
    ...
    const mustReload = reloadData || bboxKey !== lastViewStateRef.current.bboxKey || activeDataset !== lastViewStateRef.current.activeDataset;
